...
impl PumpkinRunner {
    ...
    async fn setup_pumpkin_repo(&self, force: bool) -> Result<()> {
        if self.pumpkin_dir.exists() {
            if force {
                ...
                fs::remove_dir_all(&self.pumpkin_dir).await
                    .context("Failed to remove existing Pumpkin directory")?;
            } else {
                println!("{}", "Pumpkin repository already exists, pulling latest changes...".blue());
                self.git_pull().await?;
                return Ok(());
            }
        }
    ...
    async fn git_pull(&self) -> Result<()> {
        ...
        if !output.status.success() {
            println!("{}", "Git pull failed, continuing with existing version...".yellow());
        } else {
    ...
    async fn build_current_project(&self) -> Result<()> {
        ...
        if cfg!(target_os = "windows") {
            args.push("--release");
            println!("{}", "  Windows detected: Using release build for plugin compatibility".yellow());
        }
        ...
        if !output.status.success() {
            anyhow::bail!("Current plugin build failed: {}", String::from_utf8_lossy(&output.stderr));
        }
    ...
    async fn build_pumpkin_server(&self) -> Result<()> {
        ...
        if !output.status.success() {
            anyhow::bail!("Pumpkin server build failed: {}", String::from_utf8_lossy(&output.stderr));
        }
    ...
    async fn copy_artifacts(&self) -> Result<()> {
        ...
        if pumpkin_binary.exists() {
            ...
            fs::copy(&pumpkin_binary, &dest).await
    ...
    async fn copy_plugin_artifact(&self, name: &str) -> Result<()> {
        let build_dir = if cfg!(target_os = "windows") { "release" } else { "debug" };
        ...
        let plugin_path = self.current_dir.join(format!("target/{}/{}", build_dir, plugin_filename));
        ...
        if plugin_path.exists() {
            ...
            fs::copy(&plugin_path, &dest).await
                .context("Failed to copy plugin file")?;
            println!("{}", format!("  Copied plugin {} to plugins/", plugin_filename).green());
        } else {
            println!("{}", format!("  Plugin {} not found at {}", plugin_filename, plugin_path.display()).yellow());
        }
    ...
    async fn run_server(&self) -> Result<()> {
        ...
        println!("{}", "Server is starting... (Press Ctrl+C to stop)".green().bold());
